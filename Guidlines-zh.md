...
## 1. 简介
作为设计原则，微软 REST API 指南鼓励应用程序开发人员通过 RESTful HTTP 接口访问资源。
为了按照 微软 REST API 指南为平台上的开发人员提供尽可能流畅的体验，REST API 应遵循一致的设计指南，以便轻松、直观地使用它们。

本文档确立了 微软 REST API 应遵循的准则，以便一致地开发 RESTful 接口。

## 2. 目录
<!-- TOC depthFrom:2 depthTo:4 orderedList:false updateOnSave:false withLinks:true -->

- [微软REST API指南工作组](#微软-REST-API-指南工作组)
- [1. 简介](#1-简介)
- [2. 目录](#2-目录)
- [3. 介绍](#3-介绍)
    - [3.1. 推荐阅读](#31-推荐阅读)
- [4. 指南解释](#4-指南解释)
    - [4.1. 指南的适用范围](#41-指南的适用范围)
...
- [7. 一致性基础](#7-一致性基础)
    - [7.1. URL 结构](#71-url-结构)
    - [7.2. URL 长度](#72-url-长度)
    - [7.3. 规范标识符](#73-规范标识符)
    - [7.4. 支持的方法](#74-支持的方法)
        - [7.4.1. POST](#741-post)
        - [7.4.2. PATCH](#742-patch)
        - [7.4.3. 通过PATCH（UPSERT语义）创建资源](#743-creating-resources-via-patch-upsert-semantics)
        - [7.4.4. 选项和链接headers](#744-options-and-link-headers)
    - [7.5. 标准请求headers](#75-standard-request-headers)
    - [7.6. 标准响应headers](#76-standard-response-headers)
    - [7.7. 自定义headers](#77-custom-headers)
    - [7.8. 指定headers作为query参数](#78-specifying-headers-as-query-parameters)
    - [7.9. PII 参数](#79-pii-parameters)
    - [7.10. 响应格式](#710-response-formats)
        - [7.10.1. 客户端指定响应格式](#7101-clients-specified-response-format)
        - [7.10.2. 错误情况响应](#7102-error-condition-responses)
    - [7.11. HTTP状态码](#711-http-status-codes)
    - [7.12. 客户端库可选项](#712-client-library-optional)
- [8. CORS](#8-cors)
    - [8.1. 客户端指导](#81-client-guidance)
        - [8.1.1. 避免预检](#811-avoiding-preflight)
    - [8.2. 服务指导](#82-service-guidance)
- [9. 集合](#9-collections)
    - [9.1. 项目键](#91-item-keys)
    - [9.2. 序列化](#92-serialization)
    - [9.3. URL模式的集合](#93-collection-url-patterns)
        - [9.3.1. 嵌套集合和属性](#931-nested-collections-and-properties)
    - [9.4. 大集合](#94-big-collections)
    - [9.5. 更改集合](#95-changing-collections)
    - [9.6. 集合排序](#96-sorting-collections)
        - [9.6.1. 排序表达式的解释](#961-interpreting-a-sorting-expression)
    - [9.7. 过滤](#97-filtering)
        - [9.7.1. 过滤器操作](#971-filter-operations)
        - [9.7.2. 运算符实例](#972-operator-examples)
        - [9.7.3. 运算符优先级](#973-operator-precedence)
    - [9.8. 分页](#98-pagination)
        - [9.8.1. 服务端驱动的分页](#981-server-driven-paging)
        - [9.8.2. 客户端驱动的分页](#982-client-driven-paging)
        - [9.8.3. 其它注意事项](#983-additional-considerations)
    - [9.9. 复合集合操作](#99-compound-collection-operations)
- [10. 增量查询](#10-delta-queries)
    - [10.1. 增量链接](#101-delta-links)
    - [10.2. 实体呈现](#102-entity-representation)
    - [10.3. 获取一个增量链接](#103-obtaining-a-delta-link)
    - [10.4. 增量链接响应的内容](#104-contents-of-a-delta-link-response)
    - [10.5. 使用一个增量链接](#105-using-a-delta-link)
- [11. JSON标准化](#11-json-standardizations)
    - [11.1. 基元类型的JSON格式标准化](#111-json-formatting-standardization-for-primitive-types)
    - [11.2. 日期和时间的指南](#112-guidelines-for-dates-and-times)
        - [11.2.1. 生产日期](#1121-producing-dates)
        - [11.2.2. 使用日期](#1122-consuming-dates)
        - [11.2.3. 兼容性](#1123-compatibility)
    - [11.3. 日期和时间的JSON序列化](#113-json-serialization-of-dates-and-times)
        - [11.3.1. 日期文本格式](#1131-the-dateliteral-format)
        - [11.3.2. 日期格式注释](#1132-commentary-on-date-formatting)
    - [11.4. 持续时间](#114-durations)
    - [11.5. 间隔时间](#115-intervals)
    - [11.6. 重复间隔](#116-repeating-intervals)
- [12. 版本](#12-versioning)
    - [12.1. 版本格式](#121-versioning-formats)
        - [12.1.1. 组版本](#1211-group-versioning)
    - [12.2. 何时更新版本](#122-when-to-version)
    - [12.3. 重大变更的定义](#123-definition-of-a-breaking-change)
- [13. 长时间运行的操作](#13-long-running-operations)
    - [13.1. 基于资源的长时间运行操作 (RELO)](#131-resource-based-long-running-operations-relo)
    - [13.2. 按阶段的长时间运行操作](#132-stepwise-long-running-operations)
        - [13.2.1. PUT](#1321-put)
        - [13.2.2. POST](#1322-post)
        - [13.2.3. POST, 混合模型](#1323-post-hybrid-model)
        - [13.2.4. 运营资源](#1324-operations-resource)
        - [13.2.5. 操作资源](#1325-operation-resource)
        - [13.2.6. 操作逻辑删除](#1326-operation-tombstones)
        - [13.2.7. 典型流，轮询](#1327-the-typical-flow-polling)
        - [13.2.8. 典型流，推送通知](#1328-the-typical-flow-push-notifications)
        - [13.2.9. Retry-After](#1329-retry-after)
    - [13.3. 操作结果的保留策略](#133-retention-policy-for-operation-results)
- [14. 调节，配额和限制](#14-throttling-quotas-and-limits)
    - [14.1. 原则](#141-principles)
    - [14.2. 返回码 (429 vs 503)](#142-return-codes-429-vs-503)
    - [14.3. 稍后重试和速率限制标头](#143-retry-after-and-ratelimit-headers)
    - [14.4. 服务指南](#144-service-guidance)
        - [14.4.1. 响应性](#1441-responsiveness)
        - [14.4.2. 速率限制和配额](#1442-rate-limits-and-quotas)
        - [14.4.3. 超负荷服务](#1443-overloaded-services)
        - [14.4.4. 响应示例](#1444-example-response)
    - [14.5. 访问者指南](#145-caller-guidance)
    - [14.6. 处理忽略稍后重试标头的访问者](#146-handling-callers-that-ignore-retry-after-headers)
- [15. 通过webhooks推送通知](#15-push-notifications-via-webhooks)
    - [15.1. 范围](#151-scope)
    - [15.2. 原则](#152-principles)
    - [15.3. 订阅类型](#153-types-of-subscriptions)
    - [15.4. 访问序列](#154-call-sequences)
    - [15.5. 验证订阅](#155-verifying-subscriptions)
    - [15.6. 接收通知](#156-receiving-notifications)
        - [15.6.1. 通知有效负载](#1561-notification-payload)
    - [15.7. 编程方式管理订阅](#157-managing-subscriptions-programmatically)
        - [15.7.1. 创建订阅](#1571-creating-subscriptions)
        - [15.7.2. 更新订阅](#1572-updating-subscriptions)
        - [15.7.3. 删除订阅](#1573-deleting-subscriptions)
        - [15.7.4. 枚举订阅](#1574-enumerating-subscriptions)
    - [15.8. 安全](#158-security)
- [16. 不支持的请求](#16-unsupported-requests)
    - [16.1. 基本指南](#161-essential-guidance)
    - [16.2. 功能允许列表](#162-feature-allow-list)
        - [16.2.1. 错误响应](#1621-error-response)
- [17. 命名指南](#17-naming-guidelines)
    - [17.1. 介绍](#171-approach)
    - [17.2. 外壳](#172-casing)
    - [17.3. 名称禁忌](#173-names-to-avoid)
    - [17.4. 构成复合名称](#174-forming-compound-names)
    - [17.5. 身份属性](#175-identity-properties)
    - [17.6. 日期和时间属性](#176-date-and-time-properties)
    - [17.7. 命名属性](#177-name-properties)
    - [17.8. 收藏和计数](#178-collections-and-counts)
    - [17.9. 常用属性名称](#179-common-property-names)
- [18. 附录](#18-appendix)
    - [18.1. 序列图注释](#181-sequence-diagram-notes)
        - [18.1.1. 推送通知, 用户流](#1811-push-notifications-per-user-flow)
        - [18.1.2. 推送通知, 信息流](#1812-push-notifications-firehose-flow)

<!-- /TOC -->
## 3. 介绍
开发者通过HTTP接口，可以访问绝大部分微软云平台的资源。
